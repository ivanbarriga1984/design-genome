...
import { aiPrompt, decode, emptyGene, examples, hasProgress, humanSections, json, markdown, save, slug, steps, storageKey, validateStep, type Gene, type Progress } from './model';
function restore() {
  try {
...
export default function GeneBuilder() {
  ...
  function download(){try{const url=URL.createObjectURL(new Blob([markdown(gene)],{type:'text/markdown;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=`${slug(gene.decision)}.gene.md`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);setFeedback('Markdown download requested.');}catch{setManualCopy(markdown(gene));setFeedback('Download is unavailable. Copy the Markdown below instead.');}}
