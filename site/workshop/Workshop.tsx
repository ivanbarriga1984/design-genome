...
export function Workshop({ data }: { data: WorkshopData }) {
  ...
  return <div className="wk-page dg-wrap">
    <header className="wk-intro"><span className="dg-eyebrow">DDX / Design Genome workshop</span><h1>Same task. <span>More knowledge to work from.</span></h1><p>Without organizational intelligence, the system has to make reasonable guesses. With the Genome, fewer organizational decisions are guesses.</p><p className="wk-note">Explore prepared, interactive results. No live model call. Project actions stay in this browser.</p></header>
    ...
    <section className="wk-stage" aria-labelledby="wk-stage-title" key={`${scenario}:${step}`}>
      ...
      {step === "baseline" && <>
        <div className="wk-result-heading"><span className="wk-kicker">Common inputs only</span><span>Inspect and try the result</span></div><div className="wk-single"><Result scenario={scenario} baseline /></div>
      ...
      {step === "informed" && <>
        <div className="wk-result-heading"><span className="wk-kicker">Common inputs + relevant Genome knowledge</span><span>Same request. Additional context.</span></div>
        <div className="wk-inherited"><Result scenario={scenario} /><Traces scenario={scenario} context={context} /></div>
        <details className="wk-evidence"><summary>Compare with the common-input result</summary><div className="wk-single"><Result scenario={scenario} baseline /></div></details>
        <div className="wk-breakdown"><div><h3>{scenario === "analytics" ? "Carried forward" : "Genome-informed / traceable"}</h3><p>{current.carried}</p></div><div><h3>Already reasonable</h3><p>{current.shared}</p></div><div><h3>{scenario === "analytics" ? "Still invented locally" : "Still implementation judgment"}</h3><p>{current.local}</p></div></div>
        <p className="wk-note">These results make decisions inspectable. A visual difference alone does not establish its cause or prove better model performance.</p>
