  ...
  return <div className="pr-viewport" ref={viewport}>
    <div className="pr-frame" style={{width:1600*scale,height:900*scale}}>
      <main id="presentation" ref={stage} tabIndex={-1} className={`pr-stage ${[3,7,9,15,21].includes(p.scene)?'pr-dark':''}`} style={{transform:`scale(${scale})`}} aria-label={`Scene ${p.scene}: ${scenes[p.scene-1].title}, beat ${p.beat} of ${scenes[p.scene-1].beats}`}>
        ...
        <footer className="pr-controls"><span className="pr-brand">DG.</span><span>{scenes[p.scene-1].act}</span><nav aria-label="Presentation controls"><button onClick={()=>go(move(p,-1))} disabled={p.scene===1&&p.beat===1} aria-label="Previous beat">←</button><button onClick={()=>toggle('overview')} aria-label="Open scene overview">{String(p.scene).padStart(2,'0')} / 22</button><span className="pr-beat">{p.beat} / {scenes[p.scene-1].beats}</span><button onClick={()=>go(move(p,1))} disabled={p.scene===22} aria-label="Next beat">→</button><button onClick={()=>toggle('calibration')} aria-label="Open calibration">C</button><button onClick={()=>void fullscreen()} aria-label="Toggle fullscreen">F</button><button onClick={()=>toggle('help')} aria-label="Open keyboard help">?</button></nav></footer>
